    ...
    setImage({
      ...
      preview:
        data.image.indexOf("http") === 0
          ? data.image
          : `http://localhost:4000/file/${data.image}`,
    });
    setName(data.name);
  ...
  return (
    <Container fluid>
      <div className="slider-area ">
        <div
          className="single-slider slider-height2"
      ...
      >
        <div>
          ...
          ></textarea>
          {isAdd && (
            <button type="submit" className="btn" onClick={addNews}>
              Thêm bài viết
            </button>
          )}
          {!isAdd && (
            <button type="submit" className="btn" onClick={updateNews}>
              Sửa bài viết
            </button>
          )}
        </div>
